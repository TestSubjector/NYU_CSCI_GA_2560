...
def main():
    ...
    parser.add_argument("-end",const=str, nargs="?", required=True)
    ...
    if decide_alg == "BFS":
        bfs.do_bfs(input_graph, args.start, args.end, verbose)
    elif decide_alg == "ASTAR":
        astar.do_astar(input_graph, args.start, args.end, verbose)
    elif decide_alg == "IDS":
        if args.depth == None:
            print("Depth not given for IDS. Terminating program.")
            exit(0)
        ids.do_ids(input_graph, args.start, args.end, int(args.depth), verbose)
